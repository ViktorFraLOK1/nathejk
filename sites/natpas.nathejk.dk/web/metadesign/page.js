...
function initMainSearch(){
    $(".searchModules").each(function(i){
        var htmlToAppend = '<div class="formSelectWrap"><label for="generatedSearchSelect'+i+'">Add search group</label><select class="formSelect" id="generatedSearchSelect'+i+'"><option>Pick</option>';
        $(this).find("section").not(":first-child,:last-child").each(function(){
            var thisId = $(this).attr("class")+i;
            var thisTitle = $(this).find("h1").text();
            $(this).attr("id",thisId);
            if($(this).hasClass("forceOpen")){
                htmlToAppend += '<option value="' + thisId + '" disabled="disabled">' + thisTitle + '</option>';
            } else {
                htmlToAppend += '<option value="' + thisId + '">' + thisTitle + '</option>';
                $(this).hide();
            }
            $(this).append('<span class="closeThis">Close</span>').find(".closeThis").click(function(){
                $("#generatedSearchSelect"+i).find("option[value='"+thisId+"']").removeAttr("disabled");
                $(this).parent("section").slideUp("fast").prev("section").find("input,select,textarea").eq(0).focus();
            })
        })
        htmlToAppend += '</select>';
        $(this).find("section:last-child").prepend(htmlToAppend).find("#generatedSearchSelect"+i).change(function(){
            var sectionToShow = $(this).find("option:selected").val();
            $("section#"+sectionToShow).slideDown("fast").find("input,select,textarea").eq(0).focus();
            $(this).find("option:selected").attr("disabled","disabled");
            $("#generatedSearchSelect"+i).find("option:first-child").attr("selected","selected");
        });
    })
...
function initKeyboardNav(){
   	...
   	// main nav (1-9)
    $("header").find(".primary > ul > li").each(function(i){
        // define keystroke
        var keys = "ctrl+shift+"+(i+1);
        // add title attribute for hover effect
        $(this).attr("title",keys);
        // set variable depending on if the li contains subnav
        var containsSelect = false;
        if($(this).children(".formSelectWrap").length > 0)  containsSelect = true;
        // assign keystrokes
        if(!containsSelect){
            var path = $(this).find("a").attr("href");
            $(document).bind('keydown', keys, function(evt){
                evt.stopPropagation( );  
        		evt.preventDefault( );
        	...
        	var selectToFocus = $(this).find("select");
            $(document).bind('keydown', keys, function(evt){
                evt.stopPropagation( );  
        		evt.preventDefault( );
        		selectToFocus.focus();
    ...
    // toggle menu (m)
    $(document).bind('keydown', 'ctrl+shift+m', function(evt){
        if($("#toggleProductList").length == 0) return false;
        evt.stopPropagation( );  
		evt.preventDefault( );
        var el = $("#toggleProductList");
        toggleMenu(el);
        return false;
    });
    // focus first form field (f)
    $(document).bind('keydown', 'ctrl+shift+f', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
        $("#contentContainer").find("input,select,textarea").not(":disabled").first().focus();
        return false;
    });
    // highlight first element of first multipurpose pane
    $(document).bind('keydown', 'ctrl+shift+q', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		if($(".dualColumn").length>0) $(".pseudoFocus").removeClass("pseudoFocus");
		var el = $("#searchResults").find(".column").first().addClass("pseudoFocus").find(".wrap > *");
		if(el.is("table")){
			if(el.find(".keyboardActive").length == 0) $(el).find("tbody").find("tr:first-child").addClass("keyboardActive");
		} else {
			$(el).find("input,select,a,textarea").first().focus();
		} 
        return false;
    });
    
    // highlight first element of second multipurpose pane
    $(document).bind('keydown', 'ctrl+shift+w', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		if($(".dualColumn").length>0){
			$(".pseudoFocus").removeClass("pseudoFocus");
			var el = $("#searchResults").find(".column:gt(0)").addClass("pseudoFocus").find(".wrap > *");
			if(el.is("table")){
				if(el.find(".keyboardActive").length == 0) $(el).find("tbody").find("tr:first-child").addClass("keyboardActive");
			} else {
				$(el).find("input,select,a,textarea").first().focus();
    ...
    // move to next item in table (k)
    $(document).bind('keydown', 'ctrl+shift+k', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		var el = $(".pseudoFocus tr.keyboardActive");
		if(el.length == 0 && $("table").length != 0){
			$("table").first().parents(".column").addClass("pseudoFocus").find("tbody").find("tr:first-child").addClass("keyboardActive");
		} else {
			if(el.next("tr").length == 0){
				return false;
			}
			el.removeClass("keyboardActive").next("tr").addClass("keyboardActive");
		}
		     
        return false;
    });
    // move to prev item in table (j)
    $(document).bind('keydown', 'ctrl+shift+j', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		var el = $(".pseudoFocus tr.keyboardActive");
		if(el.prev("tr").length == 0){
			return false;
		}
		el.removeClass("keyboardActive").prev("tr").addClass("keyboardActive");        
        return false;
    });
    // select checkbox of active keyboard element in table (x)
    $(document).bind('keydown', 'ctrl+shift+x', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		$(".pseudoFocus tr.keyboardActive").find("td:first-child").find("input").click();        
        return false;
    });
    // open item currently highlighted
    $(document).bind('keydown', 'ctrl+shift+o', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		document.location = $(".pseudoFocus tr.keyboardActive").find("td.title").find("a").attr("href");        
        return false;
    });
    // edit (saved searches)
    $(document).bind('keydown', 'ctrl+shift+e', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		if($(".pseudoFocus").has(".savedSearches").length>0){
			location.href = $(".pseudoFocus").find(".keyboardActive").find(".edit").find("a").attr("href");
		}
		return false;
    });
    // delete (saved searches)
    $(document).bind('keydown', 'ctrl+shift+d', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		if($(".pseudoFocus").has(".savedSearches").length>0){
			location.href = $(".pseudoFocus").find(".keyboardActive").find(".delete").find("a").attr("href");
		}
		return false;
    });
    // rename (saved searches)
    $(document).bind('keydown', 'ctrl+shift+r', function(evt){
        evt.stopPropagation( );  
		evt.preventDefault( );
		if($(".pseudoFocus").has(".savedSearches").length>0){
			location.href = $(".pseudoFocus").find(".keyboardActive").find(".rename").find("a").attr("href");
		}
		return false;
    });
...
function initToolTable(){
    $(".toolTable").not("initialised").each(function(i){
        // add class to differ initial tables from dom created
        $(this).addClass("initialised");
        
        
        // add class to table row that has checked checkboxes
        $(this).find("tbody tr").has("input:checked").addClass("active");
        // add class to table row when checkbox is marked
        $(this).find("tbody .formCheckbox").live("click",function(){
            $(this).parents("tr").toggleClass("active");
        });
        // add a check all checkbox in headers
        $(this).find("th.formCheckboxWrap").append('<input type="checkbox" class="formCheckbox" title="select all" />').find(".formCheckbox").change(function(){
            if($(this).is(":checked")){
                $(this).parents("table").find("tbody tr").addClass("active").find(".formCheckbox").attr("checked","checked");
            } else {
                $(this).parents("table").find("tbody tr").removeClass("active").find(".formCheckbox").removeAttr("checked");
            }
        });
        ...
        // get which fields are dates
        headerset = {
           0: { 
                sorter: false 
           }
        };
        $(this).find("th.date").each(function(){
        	headerset[$(this).index()] = { sorter: 'DD/MM/YYYY' };
        })
        $(this).tablesorter({
			sortList: [[1,0]],
			headers: headerset
...
function initContainer(){
    $(".productWrap").find("section.productList").addClass("open").prepend('<a id="toggleProductList" href="#" class="open">Hide list</a>').find("#toggleProductList").click(function(){
		toggleMenu(this);
    })
}
...
function toggleMenu(el){
	if($(el).hasClass("open")){
        $(el).html("Show list").removeClass("open").addClass("closed").parents("section").removeClass("open").addClass("closed").find("h1,table").hide();
        return false;
    } else {
        $(el).html("Hide list").removeClass("closed").addClass("open").parents("section").removeClass("closed").addClass("open").find("h1,table").show();
        return false;
    }
}
...
function initFancyBoxes(){
    $("a[class^='fancybox']").each(function(){
        var fullsize = $(this).attr("class").substr(8, $(this).attr("class").length );
        var sizes = fullsize.split("x");
        var theLink = $(this).attr("href");
        var toSerialize = false;
        if(isNaN(sizes[0])){
        	sizes[0] = $("body").width();
        	sizes[1] = $("body").height(); 
        }
        // check if this needs to be serialized
        if($(this).hasClass("confirm") || $(this).hasClass("saveSearch")){
        ...
        $(this).fancybox({
        	'onStart': function(){
            	if(toSerialize == true) {
					$(this).attr("href", theLink + $("#searchResults form").serialize());         	
            	}    	
            },
...
function initSelectNav(){
	$("header nav.primary").find("select").change(function(){
...
function initForms(){
    ...
    $(".clonable").each(function(i){
    	$(this).attr("id","clonable"+i);
    	
    	var wrapString = '<div class="cloneOrg" />';
    	if($(this).has(".cloneOrg").length > 0) wrapString = ''; 
    	
    	$(this).data("cloneCounter",0).wrapInner(wrapString).append('<div class="cloneTools"><button class="addClone">Add '+$(this).find("h1:first").text()+'</button></div>').find(".cloneTools").click(function(){
	    	$("#clonable"+i).data("cloneCounter", ($("#clonable"+i).data("cloneCounter")+1));
	    	var el = $(this).parents(".clonable").find(".cloneOrg").clone();
	    	el.removeClass("cloneOrg");	    	
	    	// reset elements
	    	el.find("input").attr("value","").removeClass("hasDatepicker");
	    	el.find("option[selected]").removeAttr("selected");
	    	el.find("textarea").html("");
	    	
	    	
	    	el.find("[id]").each(function(){
	    		var newId = 'c'+ $("#clonable"+i).data("cloneCounter") + $(this).attr("id");
	    		$(this).attr("id", newId);
	    		$(this).prev("label").attr("for",newId);
	    	})
	    	el.insertBefore($("#clonable"+i+" .cloneTools"));
	    	el.find("input,select,textarea").first().focus();
	    	el.find(".datePicker").datepicker({ dateFormat: 'dd/mm/yy' });
	    	return false;
	    });
    })
    
    // assign functionality to cancel-links
    $(".cancelLink").each(function(){
    	var inFancybox = $(this).parents(".fancyboxContent").length;
    	if(inFancybox > 0) $(this).click(function(){
    ...
    // mask inputs
    $("input.duration").mask("99:99:99",{placeholder:" "});
    $("input.durationExact").mask("99:99:99.9?9",{placeholder:" "});
    
    // assign form submission functionality to links
    $(".toolTableTools a.submitForm").each(function(){
    	$(this).click(function(){
    		var actionUrl = $(this).attr("href");
    		$("#searchResults form").attr("action",actionUrl).submit();
    		return false;
    	})
    })
    
    // submit form on change
    $(".formSelect.doSubmit").change(function(){
    	var theValue = $(this).find("option:selected").val();
    	var theText = $(this).find("option:selected").html();
    	if(theValue != theText) $(this).parents("form").submit();
    })
}
...
function initOrderTable(){
    $("#orderTable").map(function(){
        var listOne = $(this).find(".lists").find("ul:first-child");
        var listTwo = $(this).find(".lists").find("ul:last-child")
        var listOneLength = listOne.find("li").length;
        var listTwoLength = listTwo.find("li").length;
        var emptyVal = 0;
        if(listOneLength != listTwoLength){
            var diff = Math.abs(listOneLength-listTwoLength);
            var listToAppend = (listOneLength > listTwoLength) ? listTwo : listOne;
            var htmlString="";
            for(i=0;i<diff;i++){
                htmlString += '<li id="empty'+i+'_'+i+'" class="empty"></li>';
                emptyVal = i+1;
            }
            listToAppend.append(htmlString);
        }
        $("#orderTable").find(".formSubmitWrap").append('<span>Add Row</span>').find("span").data('counter',emptyVal).click(function(){
            $("#orderTable").find("ul").each(function(i){
                $(this).append('<li id="empty'+i+'_'+emptyVal+'" class="empty"></li>');
                $("#"+$(this).attr("id")+"order").attr("value", $(this).sortable('serialize'))
            })
            emptyVal += 1;
            $(this).data('counter',emptyVal);
        })
        
        $(this).find("ul").sortable({
            placeholder: "ui-state-highlight",
            create: function() {
                var hiddenInput = "#" + $(this).attr("id") + "order";
                $(hiddenInput).attr("value",$(this).sortable('serialize'));
            },
            stop: function() {
                var hiddenInput = "#" + $(this).attr("id") + "order";
                $(hiddenInput).attr("value",$(this).sortable('serialize'));
...
/* jquery-ui-1.8.11.custom.min.js|customers */
/*!
 * jQuery UI 1.8.11
 *
...
b.dequeue()})})}})(jQuery);
;
/* jquery.fancybox-1.3.4.pack.js|customers */
/*
...
/* jquery.maskedinput-1.2.2.min.js|customers */
/*
...
/* jquery.tablesorter.min.js|customers */
